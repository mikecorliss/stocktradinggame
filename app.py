...
app = Flask(__name__)
app.secret_key = "stock-trader-2026-secret-key-change-in-prod"
...
def init_user_game(user):
    if not user.stocks:
        ...
        user.day = 0
        user.price_history = {
            sym: [{"date": "May 14", "price": data["price"]}]
            for sym, data in user.stocks.items()
        }
        db.session.commit()

def get_current_date(user):
    start = date.fromisoformat(user.start_date)
    return (start + timedelta(days=user.day)).strftime("%A, %B %d, %Y")
...
def next_day():
    ...
    if random.random() < 0.22:
        events = [
            "🚀 TECH RALLY! Prices surge across the board",
            "📉 MARKET DIP – Broad sell-off",
            "⚡ TSLA VOLATILITY – Huge move",
            "🍎 Apple Event – Stock jumps"
        ]
        news = random.choice(events)
        if "surge" in news:
            for s in user.stocks: user.stocks[s]["price"] = round(user.stocks[s]["price"] * 1.09, 2)
        elif "dip" in news:
            for s in user.stocks: user.stocks[s]["price"] = round(user.stocks[s]["price"] * 0.94, 2)
        elif "TSLA" in news:
            user.stocks["TSLA"]["price"] = round(user.stocks["TSLA"]["price"] * random.uniform(1.08, 1.22), 2)
        elif "Apple" in news:
            user.stocks["AAPL"]["price"] = round(user.stocks["AAPL"]["price"] * random.uniform(1.06, 1.15), 2)
        update_prices(user)
